...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_navsat_fix1 = n.subscribe("/gps1/fix",1,&navfix_gps1_Callback);  // front gps 
  ros::Subscriber sub_navsat_fix2 = n.subscribe("/gps2/fix",1,&navfix_gps2_Callback);  // rear  gps
  ...
  ros::Publisher utm1_pub     = n.advertise<geometry_msgs::Pose2D>("/gps_utm_pos1",1);   
  ros::Publisher utm2_pub     = n.advertise<geometry_msgs::Pose2D>("/gps_utm_pos2",1); 
  ...
  {	
	...
	if((fix1 == 2) && (fix2==2) )  // if 2 gps's are all fixed
	{
	   ...
	   if(use_two_gps == 1)
	   {
		    gps_heading_angle.data = h_angle = atan2f(diff_y,diff_x);
		    Pose1.theta = Pose2.theta = h_angle;
	   }
	   else 
	   {
		   if(use_imu_yaw_angle == 1) h_angle = imu_yaw;		   
		   Pose1.theta = Pose2.theta = h_angle;
	   }
	   ...
	   utm1_pub.publish(utm2);
	   
	   gps_odom1.header.stamp = current_time;
       gps_odom1.header.frame_id = "gps1_odom";
